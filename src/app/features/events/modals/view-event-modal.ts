...
@Component({
  ...
  styles: [`
    ...
    .close-btn:hover { background:rgba(255,255,255,.08) !important; color:var(--text) !important }

    .edit-btn { transition:box-shadow .25s,transform .2s }
    .edit-btn:hover { box-shadow:0 0 44px rgba(255,68,51,.5) !important; transform:translateY(-1px) }

    .join-link { transition:opacity .2s }
    .join-link:hover { opacity:.75 }
  `],

  template: `
<!-- BACKDROP -->
<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 backdrop-in"
     style="background:rgba(0,0,0,.82);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px)"
     (click)="onClose()">

<!-- MODAL -->
<div class="modal-enter relative w-full sm:max-w-[620px] max-h-[96dvh] sm:max-h-[90vh] flex flex-col overflow-hidden rounded-t-[28px] sm:rounded-[28px]"
  ...
  <div class="relative shrink-0 overflow-hidden rounded-t-[28px]" style="height:240px">
    ...
    @if (event().event_img_url) {
      ...
      <div class="absolute inset-0" [style]="heroGradient()">
        <div class="absolute inset-0"
             style="background-image:url(data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.04'/%3E%3C/svg%3E);background-size:256px;mix-blend-mode:overlay"></div>
        <div class="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none select-none">
          <span style="font-family:var(--fd);font-size:8rem;letter-spacing:.06em;color:transparent;-webkit-text-stroke:1px rgba(242,238,230,.04);white-space:nowrap;padding:0 2rem">
    ...
    <div class="absolute bottom-4 left-5 flex items-center gap-2 fade-up" style="animation-delay:.1s">
      @if (isUpcoming()) {
        <span style="display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:100px;font-family:var(--fm);font-size:.61rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600;background:rgba(16,185,129,.12);color:#10b981;border:1px solid rgba(16,185,129,.26)">
          <span style="width:5px;height:5px;border-radius:50%;background:#10b981;box-shadow:0 0 7px #10b981;flex-shrink:0"></span>
          Upcoming
        </span>
      } @else {
        <span style="padding:5px 12px;border-radius:100px;font-family:var(--fm);font-size:.61rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600;background:rgba(113,113,122,.12);color:#71717a;border:1px solid rgba(113,113,122,.2)">
          Ended
        </span>
      }
      @if (event().event_location_type === EventLocationType.Online) {
        <span style="padding:5px 12px;border-radius:100px;font-family:var(--fm);font-size:.61rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600;background:var(--gold-dim);color:var(--gold);border:1px solid var(--gold-brd)">
          💻 Online
        </span>
      } @else {
        <span style="padding:5px 12px;border-radius:100px;font-family:var(--fm);font-size:.61rem;letter-spacing:.1em;text-transform:uppercase;font-weight:600;background:var(--coral-dim);color:var(--coral);border:1px solid var(--coral-brd)">
          📍 In-Person
        </span>
      }
    </div>
    ...
    <div style="padding:1.5rem;display:flex;flex-direction:column;gap:1.35rem">
      ...
      <div class="fade-up" style="animation-delay:.05s;display:flex;flex-direction:column;gap:.32rem">
        @if (event().category?.name) {
          <span style="font-family:var(--fm);font-size:.63rem;letter-spacing:.16em;text-transform:uppercase;color:var(--coral)">
            {{ event().category?.name }}
          </span>
        }
        <h2 style="font-family:var(--fd);font-size:clamp(1.9rem,6vw,2.7rem);letter-spacing:.03em;color:var(--text);margin:0;line-height:.96">
          {{ event().title }}
        </h2>
      ...
      @if (event().description) {
        <p class="fade-up" style="animation-delay:.08s;font-size:.87rem;color:var(--muted);line-height:1.75;margin:0;font-weight:300">
          {{ event().description }}
        </p>
      }
      ...
      <div class="fade-up" style="animation-delay:.12s;display:grid;grid-template-columns:1fr 1fr;gap:.7rem">
        ...
        <div class="tile" style="grid-column:span 2;display:flex;align-items:flex-start;gap:.82rem;padding:.95rem 1.1rem;border-radius:14px;background:rgba(255,255,255,.022);border:1px solid var(--border)">
          ...
          <div style="min-width:0">
            <p style="font-family:var(--fm);font-size:.59rem;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin:0 0 4px">Date &amp; Time</p>
        ...
        <div class="tile" style="grid-column:span 2;display:flex;align-items:flex-start;gap:.82rem;padding:.95rem 1.1rem;border-radius:14px"
          ...
          <div style="min-width:0;flex:1">
            <p style="font-family:var(--fm);font-size:.59rem;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin:0 0 4px">
...
@if (event().event_type !== undefined) {
  <div class="tile" style="display:flex;align-items:center;gap:.7rem;padding:.85rem .95rem;border-radius:14px;background:rgba(255,255,255,.022);border:1px solid var(--border)">
    ...
    <div>
      <p style="font-family:var(--fm);font-size:.57rem;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin:0 0 3px">Type</p>
      <p style="font-size:.84rem;font-weight:600;color:var(--text);margin:0">{{ EventType[event().event_type] }}</p>
    </div>
  </div>
}
        ...
        @if (event().category?.name) {
          <div class="tile" style="display:flex;align-items:center;gap:.7rem;padding:.85rem .95rem;border-radius:14px;background:rgba(255,255,255,.022);border:1px solid var(--border)">
            ...
            <div>
              <p style="font-family:var(--fm);font-size:.57rem;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin:0 0 3px">Category</p>
  ...
  <div style="flex-shrink:0;padding:.95rem 1.5rem;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:.75rem;background:rgba(6,6,8,.9);backdrop-filter:blur(14px)">
    ...
    <button (click)="onClose()"
            style="padding:.58rem 1.2rem;border-radius:100px;font-family:var(--fb);font-size:.82rem;font-weight:500;color:var(--muted);background:transparent;border:1px solid var(--border);cursor:pointer;transition:all .2s"
            onmouseenter="this.style.color='var(--text)';this.style.borderColor='var(--border-hi)';this.style.background='rgba(255,255,255,.04)'"
            onmouseleave="this.style.color='var(--muted)';this.style.borderColor='var(--border)';this.style.background='transparent'">
      Close
    </button>

    <button class="edit-btn" (click)="onEdit()"
            style="display:inline-flex;align-items:center;gap:8px;padding:.62rem 1.4rem;border-radius:100px;font-family:var(--fb);font-weight:700;font-size:.85rem;color:#fff;background:var(--coral);border:none;cursor:pointer;box-shadow:0 0 26px rgba(255,68,51,.3);position:relative;overflow:hidden">
