...
function getUserIdFromToken(): number | null {
 try {
   ...
   if (!raw.startsWith('eyJ')) {
     ...
     jwt = parsed?.token ?? parsed?.accessToken ?? parsed?.access_token ?? parsed?.jwt ?? '';
   }
   if (!jwt) return null;
...
export class MyCreatedMeetupsPage implements OnInit, OnDestroy {
 ...
 private resolveUserId(): number | null {
   return this.auth.getUserProfile()?.id ?? getUserIdFromToken();
 }
 ...
 submitForm() {
   ...
   if (this.modalMode() === 'create') {
     const dto: CreateMeetupDto = {
       ...
       city: this.form.city.trim() || null, region: this.form.region.trim() || null,
       street: this.form.street.trim() || null, nameOfPlace: this.form.nameOfPlace.trim() || null,
       ...
       categoryId: this.form.categoryId!, managerId: userId,
     };
     this.svc.createMeetup(dto).pipe(
       catchError(err => { this.formError.set(err?.error?.message ?? 'Failed to create meetup.'); this.saving.set(false); return of(null); }),
       takeUntil(this.d$),
     ).subscribe(res => { if (!res) return; this.saving.set(false); this.modalOpen.set(false); this.showToast('Meetup created! 🎉', 'success'); this.load(); });
     ...
     this.svc.updateMeetup(this.editTarget()!.id, dto).pipe(
       catchError(err => { this.formError.set(err?.error?.message ?? 'Failed to update meetup.'); this.saving.set(false); return of(null); }),
       takeUntil(this.d$),
     ).subscribe(res => { if (!res) return; this.saving.set(false); this.modalOpen.set(false); this.showToast('Meetup updated ✓', 'success'); this.load(); });
   }
 }
 ...
 doDelete() {
   const m = this.deleteTarget(); if (!m) return;
   this.deletingId.set(m.id);
   this.svc.deleteMeetup(m.id).pipe(
     catchError(err => { this.showToast(err?.error?.message ?? 'Failed to delete.', 'error'); this.deletingId.set(null); this.deleteTarget.set(null); return of(null); }),
     takeUntil(this.d$),
   ).subscribe(res => {
     this.deletingId.set(null); this.deleteTarget.set(null);
     if (res === null) return;
